...
当前 SDK 的 `checkArbitrage()` / `longArbProfit` / `shortArbProfit` 保留为 gross 初筛。生产执行请使用 `checkArbitrageWithFees()`、`MarketService.detectArbitrageNet()` 或 `TradingService.estimateBinaryArbitrageFees()`。
...
- [ ] 执行路径必须显式使用 FOK/IOC/批量下单、最小利润保护、最大滑点保护和失败处理
